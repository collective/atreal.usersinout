...
class UsersInOut (BrowserView):
    ...
    def __call__(self):
        ...
        if (method != 'POST')\
                or not int(self.request.form.get('form.submitted', 0)):
            return self.index()

        if self.request.form.get('form.button.Cancel'):
            return self.request.response.redirect(
                '%s/plone_control_panel' %
    ...
    def importUsers(self):
        ...
        for line in reader:
            ...
            try:
                groups = [g.strip()
                          for g in datas.pop('groups').split(',') if g]
                for group in groups:
        ...
        if invalidLines:
            ...
            msg = _(
                'Some errors occured. Please check your CSV syntax and retry.')
            type = 'error'
        else:
